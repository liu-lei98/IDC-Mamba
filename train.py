...
def train(epoch, logger):
    ...
    for i in train_tqdm:
        ...
        # scheduler.step()
        train_tqdm.set_description(desc='[epoch: %d][lr: %.6f][loss: %.6f][mean_loss: %.6f]'%(epoch,scheduler.get_last_lr()[0],loss,epoch_loss / (i+1)))
...
def test(epoch, logger):
    ...
    input_meas = init_meas(test_gt, Phi_batch_test, opt.input_setting).to(device)
    model.eval()
    begin = time.time()
    image_log = {}
    pred = []
